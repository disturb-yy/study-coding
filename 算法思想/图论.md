...
<img src="../img/202301122041135.jpeg" alt="img" style="zoom: 67%;" />
...
**所以说，图本质上就是个高级点的多叉树而已，适用于树的 DFS/BFS 遍历算法，全部适用于图。**
...
**如果某两个节点连通，则让其中的（任意）一个节点的根节点接到另一个节点的根节点上（即修改其中一个节点的父节点为另外一个节点的父节点）**：
...
<img src="../img/1.png" alt="img" style="zoom: 25%;" />
...
所谓最小生成树（Minimum Spanning Tree, MST），就是图中若干边的集合，你要保证这些边：
...
再来看看力扣[1584. 连接所有点的最小费用 - 力扣（LeetCode）](https://leetcode.cn/problems/min-cost-to-connect-all-points/)
...
func minCostConnectPoints(points [][]int) int {
    ...
    for i := 0; i < n; i++ {
        for j := i+1; j < n; j++ {
            ...
            // 保存各点之间的的距离
            edges = append(edges, [3]int{i, j, abs(xi-xj)+abs(yi-yj)})
        }
    }
...
<img src="../img/1-16763578122113.jpeg" alt="img" style="zoom:50%;" />
...
<img src="../img/2-16763580483056.jpeg" alt="img" style="zoom: 33%;" />

那么，你肯定可以找到若干「切分」方式，将这棵最小生成树切成两棵子树。比如下面这种切分：

<img src="../img/3-16763580639699.jpeg" alt="img" style="zoom: 33%;" />
...
<img src="../img/6-167635835007122.jpeg" alt="img" style="zoom:33%;" />

是否可以快速算出 `cut({A, B, C})`，也就是节点 `A, B, C` 的所有「横切边」有哪些？

<img src="../img/8-167635844876225.jpeg" alt="img" style="zoom:33%;" />
...
<img src="../img/9.jpeg" alt="img" style="zoom:33%;" />
...
通过一个图的权值[矩阵](https://baike.baidu.com/item/矩阵?fromModule=lemma_inlink)求出它的每两点间的[最短路径](https://baike.baidu.com/item/最短路径?fromModule=lemma_inlink)矩阵。 [3] 

从图的带权[邻接矩阵](https://baike.baidu.com/item/邻接矩阵?fromModule=lemma_inlink)A=[a(i,j)] n×n开始，迭代地进行n次更新，即由矩阵D(0)=A，按一个公式，构造出矩阵D(1)；又用同样地公式由D(1)构造出D(2)；……；最后又用同样的公式由D(n-1)构造出矩阵D(n)。矩阵D(n)的i行j列元素便是i号顶点到j号顶点的最短路径长度，称D(n)为图的[距离矩阵](https://baike.baidu.com/item/距离矩阵?fromModule=lemma_inlink)，同时还可引入一个后继节点矩阵path来记录两点间的最短路径。

采用松弛技术（[松弛操作](https://baike.baidu.com/item/松弛操作?fromModule=lemma_inlink)），对在i和j之间的所有其他点进行一次松弛。所以时间复杂度为O(n^3);

##### 状态转移方程

其[状态转移方程](https://baike.baidu.com/item/状态转移方程?fromModule=lemma_inlink)如下： map[i,j]:=min{map[i,k]+map[k,j],map[i,j]}；
...
func floyd(n int, edges [][]int) {
    ...
    for _, e := range edges {
        f, t, w := e[0], e[1], e[2]
        graph[f][t], graph[t][f] = w, w 
    }
    // 更新状态方程，寻找i到j直接除了直达以为，有无更短路径
    for k := 0; k < n; k++ {  // i到j经过k
        graph[k][k] = 0 
        for i := 0; i < n; i++ {
            for j := 0; j < n; j++ {
