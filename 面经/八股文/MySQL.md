...
<img src="../img/mysql查询流程.png" alt="查询语句执行流程" style="zoom: 67%;" />
...
如果 SQL 是**查询语句（select 语句）**，MySQL 就会先去查询缓存（ Query Cache ）里查找缓存数据，看看之前有没有执行过这一条命令，这个查询缓存是以 key-value 形式保存在内存中的，key 为 SQL 查询语句，value 为 SQL 语句查询的结果。
...
- B+树是一种多路搜索树，是在B树的基础上进行的改进。

- 只有叶子节点（最底层的节点）才存放了数据，非叶子节点（其他上层节）仅用来存放目录项作为索引。
- 非叶子节点分为不同层次，通过分层来降低每一层的搜索量；
- B+ 树能够尽可能地利用节点的空间，使得树的高度更低，从而能够减少磁盘I/O次数，提高检索效率。
...
<img src="../img/a9e6a9708a6dbbcc65906d1338d2ae70.png" alt="图片" style="zoom: 50%;" />
...
比如，下面这条查询语句 where id + 1 = 10，执行计划中 type = ALL，说明是通过全表扫描的方式查询数据的：
...
<img src="../img/550dc58c222cc8d2740985e8d6497821.png" alt="图片" style="zoom:50%;" />
...
<img src="../img/e5a23e5c53ef471b947b5007866229fe-16807660267563.png" alt="img" style="zoom: 33%;" />

## 为什么要有 Buffer Pool？

<img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost4@main/mysql/innodb/bufferpool%E5%86%85%E5%AE%B9.drawio.png" alt="img" style="zoom:50%;" />
...
redo log 使用的就是 MySQL 中常见的 WAL 技术（Write-Ahead Logging），它的关键点就是先写日志，再写磁盘。

举个例子，redo log 就是一块小黑板，当有一条记录需要更新时，Innodb 引擎就会先把记录以物理日志（**对 XXX 表空间中的 YYY 数据页 ZZZ 偏移量的地方做了AAA 更新**）的形式写到黑板上，等到空闲的时候才将黑板的记录记到账本上。

<img src="../img/image-20230414003940406.png" alt="image-20230414003940406" style="zoom: 33%;" />

write_pos 是当前记录的位置，check_point 是记录的擦除位置，而中间绿色部分则是可写入新日志的位置。这样，即使 MySQL 发生异常重启（脑子懵了），之前提交的记录也不会丢失（有黑板），这种能力就叫 **crash-safe**。
...
bin log 是逻辑日志（记录xxx行的某字段的修改），其有两个文件，是以追加写的方式进行操作，当 bin log 文件写到一定大小就会切换到另一个 bin log 文件，而不是像 redo log 那样循环写的覆盖操作。
...
<img src="../img/回滚事务.png" alt="回滚事务" style="zoom: 67%;" />
...
## redo log、bin log 和 undo log 的区别

redo log 是在事务提交时写入的，所以所有提交的事务都可以恢复（保证事务的持久性），而bin log 是在提交之前写入的（先写到 binlog cache），因此可能存在未提交事务未被记录（没将 cache 中的文件写到 binlog），在事务提交的时候，执行器把 binlog cache 里的完整事务写入到 binlog 文件中，并清空 binlog cache。
